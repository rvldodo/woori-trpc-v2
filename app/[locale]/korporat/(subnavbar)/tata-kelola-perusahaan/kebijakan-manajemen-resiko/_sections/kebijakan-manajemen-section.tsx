...
export default function KebijakanManajemenSection({ l }: Props) {
  ...
  const { data, isLoading } =
    api.main.template6.template6DataByTabsId.useQuery(debounce);
  ...
  return (
    <article className="main-padding-x grid md:grid-cols-6 grid-cols-1 gap-5 py-8">
      ...
      <ScrollArea className="main-padding-x md:col-span-4 flex flex-col justify-between gap-3 h-125">
        {isLoading ? (
          <div className="w-full h-full flex justify-center items-center">
            <Spinner />
          </div>
          ...
          data?.data.map((e) => (
            <div key={e.id.toString()}>
              <TextHTML variant="body-md-regular" html={e.content?.[l] ?? ""} />
            </div>
          ))
        )}
      </ScrollArea>
