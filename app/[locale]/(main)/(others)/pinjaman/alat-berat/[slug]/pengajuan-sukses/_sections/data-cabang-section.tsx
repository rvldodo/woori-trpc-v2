...
import { CabangCard } from "@/app/[locale]/(main)/(subnavbar)/_components/branch-card";
...
export default function DataCabangSection({ l }: Props) {
  ...
  const { data: dataBranch, isLoading: isLoadingBranches } =
    api.main.branches.list_he.useQuery(
      {
        ...debounce,
      },
      {
  ...
  const { data: categoriesData, isLoading: isLoadingCategories } =
    api.main.branches.provinces_he.useQuery(debounceCategory, {
      staleTime: 1000 * 60 * 5, // Cache for 5 minutes
    });
  ...
  return (
    <section className="flex flex-col justify-start items-center gap-3">
      <div className="relative w-full md:flex hidden items-center justify-center">
        <Text
          variant="display-lg"
